...
fn safe_dump(
    ...
    let output = if output.starts_with("---") {
        output.trim_start_matches("---").trim_start_matches('\n').to_string()
    } else {
        output
    };
...
fn sort_yaml_keys(yaml: &Yaml) -> Yaml {
    match yaml {
        ...
        Yaml::Array(arr) => {
            Yaml::Array(arr.iter().map(sort_yaml_keys).collect())
        }
        other => other.clone()
    }
}
...
fn safe_dump_all(documents: &Bound<'_, PyAny>) -> PyResult<String> {
    ...
    for (i, item) in iter.enumerate() {
        ...
        // Remove the leading "---\n" that yaml-rust2 adds
        let doc_output = doc_output.trim_start_matches("---").trim_start_matches('\n');
...
"#;
    ...
    fn test_yaml_122_null() {
        ...
        for null_str in &["~", "null"] {
            ...
            assert!(matches!(&docs[0], Yaml::Null), "Failed for: {} (got {:?})", null_str, docs[0]);
        }
    ...
    fn test_yaml_122_boolean_valid() {
        for (input, expected) in &[
            ...
            assert!(matches!(&docs[0], Yaml::Boolean(b) if *b == *expected), 
                   "Failed for: {}", input);
        }
    }
    ...
    fn test_yaml_122_integer() {
        ...
        for (input, expected) in test_cases {
            let docs = YamlLoader::load_from_str(input).unwrap();
            assert!(matches!(&docs[0], Yaml::Integer(i) if *i == expected),
                   "Failed for: {} (expected {})", input, expected);
        }
    }
        ...
        for (input, expected) in test_cases {
            ...
            if let Yaml::Real(s) = &docs[0] {
                ...
                assert!((parsed - expected).abs() < 1e-10,
                       "Failed for: {} (expected {}, got {})", input, expected, parsed);
            } else {
    ...
    fn test_yaml_122_special_floats() {
        ...
        for inf_str in &[".inf", ".Inf", ".INF"] {
            ...
            if let Yaml::Real(s) = &docs[0] {
                assert!(s.to_lowercase() == ".inf" || s.to_lowercase() == "+.inf",
